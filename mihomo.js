...
function overwriteDns(params) {
  const dnsList = ["https://223.5.5.5/dns-query", "https://doh.pub/dns-query"];
...
function overwriteProxyGroups(params) {
  ...
  const allCountryTermsProxyGroupRegexs = [
    {
      name: "所有 ",
      regex: new RegExp(
        `^(?=.*${allCountryTerms})(?!.*(${excludeTerms})).*$`,
        "i"
      ),
    },
  ];
  // 流媒体以供openai使用
  const includeMediaTerms = "流媒体";
  const mediaProxyGroupRegexs = [
    {
      name: "流媒体 - 自动选择",
      regex: new RegExp(
        `^(?=.*${includeMediaTerms})(?!.*${excludeTerms}).*$`,
        "i"
      ),
    },
  ];

  const mediaProxyGroups = mediaProxyGroupRegexs
    .map((item) => ({
      name: item.name,
      type: "url-test",
  ...
  //负载均衡
  const loadBalanceProxyGroups = allCountryTermsProxyGroupRegexs
    .map((item) => ({
      name: "负载均衡",
      type: "load-balance",
      url: "https://cp.cloudflare.com",
  ...
  groups.push(...mediaProxyGroups);
  groups.push(...loadBalanceProxyGroups);
